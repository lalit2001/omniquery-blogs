...
- **Federated Query Engines**: Systems like Apache Trino that can query multiple databases (PostgreSQL, MongoDB, Snowflake, BigQuery, S3) as if they were a single source — critical for cross-system BI questions.
...
OmniQuery routes the query through Apache Trino, which:
...
Results are returned in under 500ms for most queries, rendered as a clean data table with:
- Sortable columns
...
|-----------|---------------|----------------------|
| **Time to insight** | Days to weeks | Under 500ms |
...
A question like *"Which customers who opened a support ticket also churned within 30 days?"* requires joining Zendesk data (in PostgreSQL) with subscription data (in Snowflake). Trino-powered federation in OmniQuery makes this a single, natural language question.
...
|--------|-----------------|-----------------|
| Average time to answer ad-hoc question | 3–5 days | < 500ms |
